...
async def add_to_pack(    catevent,    conv,    args,    packname,    pack,    userid,    username,    is_anim,    stfile,    emoji,    cmd,    pkang=False ):
    ...
    return pack, packname
@SU_SELVA.on(admin_cmd(pattern="صنع ملصق(?:\s|$)([\s\S]*)"))    
async def kang(args):  
    photo = None
    emojibypass = False
    is_anim = False
...
@SU_SELVA.on(admin_cmd(pattern="جلب الملصقات(?:\s|$)([\s\S]*)"))    
...
@SU_SELVA.on(admin_cmd(pattern="انشاء حزمه ملصقات(?:\s|$)([\s\S]*)"))   
...
@SU_SELVA.on(admin_cmd(pattern="جلب معلومات الملصق$"))    
...
@SU_SELVA.on(admin_cmd(pattern="ملصق ?([\s\S]*)"))    
